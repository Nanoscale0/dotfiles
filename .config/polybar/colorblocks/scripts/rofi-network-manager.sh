...
function initialization() {
	...
	if [[ -f "$DIR/rofi/networkmenu.rasi" ]]; then
		RASI_DIR="$DIR/rofi/networkmenu.rasi" 
...
function rofi_menu() {
	...
	if [[ $(nmcli device | awk '$2=="wifi" {print $1}' | wc -l) -ne "1" ]]; then
		...
		rofi -dmenu -location "$LOCATION" -p "$PROMPT" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" \
...
function ssid_manual() {
	...
	PROMPT="Enter_SSID"
	WIDTH=35
	SSID=$(	rofi -dmenu -location "$LOCATION" -p "$PROMPT" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" \
	-theme "$RASI_DIR" -theme-str '
	window{width: '"$(($WIDTH/2))"'em;}
	listview{lines: '"$LINES"';}
	')
